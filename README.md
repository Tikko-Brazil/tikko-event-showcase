...
The endpoint is authenticated, not secret-guarded: the suite logs in with `SMOKE_TEST_ADMIN_EMAIL` / `SMOKE_TEST_ADMIN_PASSWORD` — the same organizer account TC-06 through TC-09 use — and sends that bearer token. The backend answers 401 without a session, 403 to any caller whose `user.role` is not admin, and refuses the call anyway for any event outside an `is_test` organization and for any address that is not a smoke address. There is no `SMOKE_TEST_CLEANUP_TOKEN` any more; if it is still set on either side, it is unused and can be deleted.
