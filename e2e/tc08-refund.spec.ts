...
test.describe('TC-08: reembolsar ingresso pago', () => {
  test('estorna o ingresso do participante e o remove da lista de aprovados', async ({ browser }, testInfo) => {
    const email = smokeEmail('tc08-refund', testInfo.retry);

    // --- Buyer session: a real R$ 1,00 paid ticket -------------------------
    ...
    await openParticipants(organizerView, TEST_EVENT.id, { search: email, filter: 'approved' });
    const participant = participantCard(organizerView, email);
    await expect(participant, 'The paid ticket did not surface as an approved participant').toBeVisible();
    ...
    expect(payload.event_id, 'Refund must carry the event id').toBe(TEST_EVENT.id);
    ...
    // A refund flips the invite to `Refunded`, and the participants screen
    // only offers `Accepted` and `Rejected` — so the participant disappears
    // from every list the organizer can reach.
    await openParticipants(organizerView, TEST_EVENT.id, { search: email, filter: 'approved' });
    await expect(participantCard(organizerView, email)).toHaveCount(0);
    await openParticipants(organizerView, TEST_EVENT.id, { search: email, filter: 'rejected' });
    await expect(participantCard(organizerView, email)).toHaveCount(0);
    ...
    try {
      ...
      const invite = list.find((item) => Number(item.event_id) === TEST_EVENT.id);
      expect(invite, `No invite for event ${TEST_EVENT.id} in ${JSON.stringify(body)}`).toBeTruthy();
      ...
      if (data.payment_id) {
        ...
        // pinning `paid: true` here would freeze a bug in place.
      ...
      const again = await api.post(`${apiBase}/private/payment/refund`, {
        headers: { Authorization: `Bearer ${token}` },
        data: { user_id: data.user_id, event_id: TEST_EVENT.id },
      });
      expect(
        again.status(),
